...
from coalition3.statlearn.feature import plot_pred_vs_obs_core
...
def plot_var_time_series_dt0_multiquant(TRT_ID_sel, df_nonnan):
    date_of_cell = dt.datetime.strptime(TRT_ID_sel["TRT_ID"][:12], "%Y%m%d%H%M")
...
for i, pred_dt in enumerate(ls_pred_dt):
    xgb_model_path_feat = "/data/COALITION2/PicturesSatellite/results_JMZ/0_training_NOSTRADAMUS_ANN/statistical_learning/feature_selection/models/diam_23km/without_radar_t0/sample_weight/model_%i_t0diff_maxdepth6.pkl" % pred_dt
    pred_model_path     = "/data/COALITION2/PicturesSatellite/results_JMZ/0_training_NOSTRADAMUS_ANN/statistical_learning/feature_selection/models/diam_23km/without_radar_t0/sample_weight/model_%i_t0diff_maxdepth6_750feat_gain.pkl" % pred_dt
    ...
    with open(xgb_model_path_feat,"rb") as file:
    ...
    TRT_diff_pred = pred_model.predict(X_test[top_features.index[:750]]) #[xgb_model.get_booster().feature_names[:750]])
    Rank_obs, Rank_pred_XGB, Rank_pred_XGB_PM, Rank_pred_pers, Rank_pred_pers_PM, Rank_pred_diff, Diff_pred_XGB = get_obs_fcst_TRT_Rank(X_test["TRT_Rank|0"], TRT_diff_pred, y_test, X_test["TRT_Rank|-5"])
...
Rank_obs_df.columns = ["TRT_Rank_obs|%i" % pred_dt for pred_dt in np.arange(5,50,5)]
Rank_pred_XGB_df         = pd.concat(Rank_pred_XGB_ls, axis=1, sort=True)
Rank_pred_XGB_df.columns = ["TRT_Rank_pred|%i" % pred_dt for pred_dt in np.arange(5,50,5)]
Rank_pred_XGB_PM_df         = pd.concat(Rank_pred_XGB_PM_ls, axis=1, sort=True)
Rank_pred_XGB_PM_df.columns = ["TRT_Rank_pred_PM|%i" % pred_dt for pred_dt in np.arange(5,50,5)]
...
with open("TRT_Rank_obs_pred.pkl","wb") as file: pickle.dump(Rank_obs_pred_df, file, protocol=2)
with open("XGB_model_750feat_ls.pkl","wb") as file: pickle.dump(pred_model_ls, file, protocol=2)
with open("XGB_model_allfeat_ls.pkl","wb") as file: pickle.dump(xgb_model_ls, file, protocol=2)


df_R2_param_rank      = pd.concat(df_param_ls_rank,axis=0).set_index(ls_pred_dt)
df_R2_param_rank_PM   = pd.concat(df_param_ls_rank_PM,axis=0).set_index(ls_pred_dt)
df_R2_param_diff      = pd.concat(df_param_ls_diff,axis=0).set_index(ls_pred_dt)
df_R2_param_rank_pers = pd.concat(df_param_ls_rank_pers,axis=0).set_index(ls_pred_dt)
